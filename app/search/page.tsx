...
const SmartImage = memo(function SmartImage({
  ...
  const fetchAniListFallback = async () => {
    ...
    try {
      ...
      if (altPoster) setImgSrc(altPoster);
      else setImgSrc("/no-image.png");
    } catch {
  ...
  const handleError = () => fetchAniListFallback();
...
// ─── TRANSFORMER (Fix untuk JSON search terbaru) ───
function transformItem(item: any): MangaItem {
  return {
    title: item.title || "Untitled",
    slug: item.slug || item.manga_id || "",
    thumb: cleanThumb(item.thumb || item.cover_image_url || item.thumbnail || ""),
...
function SkeletonCard() {
  return (
    <div className="flex flex-col h-full">
      <div className="aspect-[3/4] w-full rounded-2xl bg-[#1c1c1c] animate-pulse border border-white/[0.04]" />
      <div className="h-3 rounded-lg bg-[#1c1c1c] w-3/4 animate-pulse mt-2.5" />
      <div className="h-2 rounded-lg bg-[#1c1c1c] w-1/2 animate-pulse mt-1.5" />
    </div>
  );
}

// ─── KARTU HASIL PENCARIAN ───
function SearchCard({ item, accentStyle }: { item: MangaItem; accentStyle: any }) {
  return (
    <Link
    ...
    >
      <div className="flex flex-col h-full">
        <div className="relative overflow-hidden rounded-2xl bg-[#141414] aspect-[3/4] mb-2 border border-white/[0.08] shadow-xl shadow-black/40 group">
          ...
          />
          {/* Ubah opacity gradient jadi 80 biar teks makin kebaca */}
...
function SearchContent() {
  ...
  useEffect(() => {
    async function fetchSearchData() {
      ...
      try {
        const res: any = await searchManga(submittedQuery);
        console.log("Search API Response:", res);

        // Fix: JSON search langsung array di res.data
        let rawData: any[] = [];
        ...
        const formattedData = rawData.map(transformItem);
        ...
        <div className="max-w-md mx-auto flex items-center gap-3 px-4 h-14">
          ...
          <div className="flex-1 relative">
            ...
            <input
              ...
              className={cn(
                "w-full bg-white/[0.03] border border-white/[0.08] rounded-xl pl-9 pr-10 py-2.5 text-sm text-white placeholder-neutral-600 focus:outline-none transition-all",
        ...
        {error && !loading && (
          <div className="flex flex-col items-center justify-center py-16 px-4 text-center bg-[#141414] rounded-2xl border border-white/[0.08] shadow-xl shadow-black/40">
            ...
            <button
              ...
              className={cn(
                "flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold text-white transition active:scale-95",
                accentStyle.bg,
                accentStyle.glow
              )}
            >
...
// ─── HALAMAN UTAMA (DIBUNGKUS SUSPENSE) ───
