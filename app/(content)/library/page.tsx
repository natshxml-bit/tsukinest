...
function cleanThumbUrl(url: string): string {
  ...
  if (url.startsWith("http") && !url.includes("wsrv.nl") && !url.includes("anilist.co") && !url.includes("kitsu.io")) {
...
// --- SISTEM CACHE KITSU GLOBAL (Mencegah over-fetching di halaman Library) ---
...
const SmartImage = memo(function SmartImage({
  ...
  if (!imgSrc || imgSrc === "/no-image.png" || attempt === "failed") {
    return (
      <div className={cn("bg-[#1c1c1c] flex flex-col items-center justify-center text-center p-2 border border-neutral-800/50 selection:bg-transparent", fill ? "absolute inset-0 w-full h-full" : "w-full h-full min-h-[120px]", className)}>
        <ImageIcon className="text-neutral-600 w-6 h-6 mb-1 opacity-70 animate-pulse" />
        <span className="text-[9px] text-neutral-500 font-medium line-clamp-2 px-1 max-w-[110px] leading-tight">{alt || title || "No Image"}</span>
      </div>
    );
  }
  ...
  return (
    <img
      ...
      className={cn(
        ...
        "transition-opacity duration-300 text-transparent", // <-- PROTEKSI STRIP JADUL / TEKS BOCOR
              ...
              return (
                ...
                >
                  ...
                  {!isEditMode && activeTab === "history" && ((manga as any).chapter_slug || manga.slug) && (
                    <Link
                      href={`/chapter/${manga.slug}/${(manga as any).chapter_slug || manga.slug}`}
